...
public:
    ...
    int room_number;
    centeredCin(langManager.getTranslation("ENTER_ROOM_NUMBER"), room_number);
    
    if (room_number >=1 && room_number <=3){
        amount = single;
        cout<< "1";
        printCentered(langManager.getTranslation("PAYMENT_Amount"));
        printCentered(langManager.getTranslation("SINGLE"));

    }
    
    else if (room_number >=4 && room_number <=7){
        amount = doubleRoom;
        cout<< "2";
        printCentered(langManager.getTranslation("PAYMENT_Amount"));
        printCentered(langManager.getTranslation("DOUBLEROOM"));
    }
    
    else if (room_number >=8 && room_number <=10){
        amount = suite;
        cout<< "3";
        printCentered(langManager.getTranslation("PAYMENT_Amount")  );
        printCentered(langManager.getTranslation("SUITE"));
    }
    
    else{
        cerr << "Failed to Failed the room\n";
        return; // Handle error
    }
